...
value eval(empty_expr const &, closure const &);
string type(empty_expr const &);
ostream & operator<<(ostream & out, empty_expr const &);
...
private:
    struct expr_concept
    {
        ...
        virtual value get_eval(closure const &) const = 0;
    };
    ...
    struct expr_model final : expr_concept
    {
        ...
        value get_eval(closure const & c) const { return eval(t,c); }
...
public:
    ...
    value get_eval(closure const & c) const { return expr_->get_eval(c); }
    ostream & out(ostream & out) const { return expr_->out(out); }
    string get_type() const { return expr_->get_type(); }
};

value eval(expression const & e, closure const & c);
string type(expression const & e);
